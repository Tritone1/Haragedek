...
const tags = ["dinner", "happy hour", "happy hour", "dinner", "dinner", "happy hour", "all day", "dinner", "lunch", "dinner", "happy hour", "all day", "dinner", "dinner", "lunch", "dinner", "dinner", "happy hour"] as const;
const photos = ["photo-1552566626-52f8b828add9", "photo-1555396273-367ea4eb4db5", "photo-1514933651103-005eec06c04b", "photo-1544148103-0773bf10d330", "photo-1517248135467-4c7edcad34c4", "photo-1551218808-94e220e084d2"];

export async function seedBakuStarterData(prisma: PrismaClient) {
  const existingVenueCount = await prisma.restaurant.count();
  if (existingVenueCount > 0) return { seeded: false, venueCount: existingVenueCount };
  ...
  for (let index = 0; index < restaurants.length; index += 1) {
    ...
    const restaurant = await prisma.restaurant.create({ data: {
      ...
      phone: `+994 50 555 ${String(1000 + index).slice(-4)}`,
  ...
  return { seeded: true, venueCount: restaurants.length };
}
